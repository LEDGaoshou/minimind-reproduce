...
if __name__ == '__main__':
    ...
    is_mini = True  # 是否为 mini 模型
    # convert torch to transformers
    torch_path = f"./out/dpo_{'mini_' if is_mini else ''}{lm_config.hidden_size}{'_moe' if lm_config.use_moe else ''}.pth"
    transformers_path = './minimind-3-dpo'
    convert_torch2transformers(torch_path, transformers_path)

    # # merge lora
    # base_torch_path = f"../out/full_sft_{lm_config.hidden_size}{'mini_' if is_mini else ''}{'_moe' if lm_config.use_moe else ''}.pth"
    # lora_path = f"../out/lora_identity_{lm_config.hidden_size}{'mini_' if is_mini else ''}{'_moe' if lm_config.use_moe else ''}.pth"
    # merged_torch_path = f"../out/merge_identity_{lm_config.hidden_size}{'mini_' if is_mini else ''}{'_moe' if lm_config.use_moe else ''}.pth"
