...
def init_model(args):
    ...
    print(
        f"MiniMind模型参数: {sum(p.numel() for p in model.parameters()) / 1e6:.2f} M(illion)"  # ！修正：原残留MiniMind命名
    )
...
def main():
    ...
    parser.add_argument(
        "--tokenizer_path",
        default="./model_learn_tokenizer",
        type=str,
        help="分词器路径（预训练模型或自定义）",
    )
