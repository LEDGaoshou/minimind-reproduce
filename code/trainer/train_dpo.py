...
if __name__ == "__main__":
    ...
    parser.add_argument("--tokenizer_path", type=str, default="./model_learn_tokenizer", help="自定义分词器路径")
    ...
    # ========== 5. 定义模型和参考模型 ==========
    model, tokenizer = init_model(lm_config, args.from_weight, tokenizer_path=args.tokenizer_path, device=args.device,is_mini=args.is_mini)
    Logger(f'策略模型总参数量：{sum(p.numel() for p in model.parameters()) / 1e6:.3f} M')
    # 初始化参考模型（ref_model冻结）
    ref_model, _ = init_model(lm_config, args.from_weight,tokenizer_path=args.tokenizer_path, device=args.device,is_mini=args.is_mini)
    ref_model.eval()
