    ...
    with open(data_path, 'r', encoding='utf-8', errors='ignore') as f:
        for i, line in enumerate(f):
            ...
            try:
                ...
                contents = [item.get('content') for item in data.get('conversations', []) if item.get('content')]       # 取出数据集中不为空的内容
                if contents:
                    '''
                    yield:生成器产出关键字。
                    它不像 return 那样一次性返回并结束函数，而是“产出一个数据，然后暂停等待”。
                    当外部的训练器请求下一个数据时，它会从暂停的地方继续执行。
                    '''
                    yield "\n".join(contents)  
...
def train_tokenizer(data_path, tokenizer_dir, vocab_size, special_tokens_num=SPECIAL_TOKENS_NUM):
    ...
    buffer_tokens = [f"<|buffer{i}|>" for i in range(1, num_buffer + 1)] # 预留一定数量的特殊token位置
    ...
    # 下面两行代码都会保存训练之后的结果，可以简单地认为是两种不同保存格式
    tokenizer.save(os.path.join(tokenizer_dir, 'tokenizer.json'))
    tokenizer.model.save(tokenizer_dir)

    # 下面的代码是为了确保在保存的tokenizer.json中，只有预定义的特殊token被标记为special=True，而其他的token都被标记为special=False
