...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels) in enumerate(
        ...
        with autocast_ctx:
            # 前向传播（labels 由模型内部计算 shift-CE loss）
            res=model(input_ids=input_ids, labels=labels)
            # 计算loss（aux_loss 为 MoE 辅助损失，当前架构未实现时兜底为 0）
            loss=res.loss + getattr(res, "aux_loss", 0.0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tokenizer_path", type=str, default="./model_learn_tokenizer", help="分词器路径（预训练模型或自定义）")
    # ========== 硬件和性能参数 ==========
    parser.add_argument(
        "--device",
        type=str,
